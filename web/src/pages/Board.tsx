...
import { buildStreamChains, chainSize, type ChainNode } from "@/lib/lineage";
              ...
              return (
                <section key={group.stream} className="rounded-xl border border-border/70 bg-secondary/20 p-4">
                  <div className="mb-3 flex flex-wrap items-center gap-2 px-1">
                    ...
                    {group.prCount > 0 ? (
                      <span className="rounded-full bg-background px-2 py-0.5 text-xs text-muted-foreground">{group.prCount} PR{group.prCount === 1 ? "" : "s"}</span>
                    ) : null}
                  </div>
